...
function addData() {
    ...
    // Validate input
    if (!id || !plate || !bluetoothDevices) {
        alert('ID, Plate, and Bluetooth Device are required.');
        return;
    }
    ...
    $.ajax({
        ...
        data: JSON.stringify({ _id: id, plate: plate, bluetooth_devices: bluetoothDevices }),
...
function fetchData() {
    $.get('/api/items', function (data) {
        ...
        data.forEach(function (item) {
            ...
            const tableRow = $('<tr>');
            tableRow.append($('<td>').text(item._id));
...
function fetchLastInserted() {
    $.get('/api/lastInserted', function (result) {
        ...
        if (result.success) {
            ...
            const dataRow = $('<tr>');
            dataRow.append($('<td>').text(lastInserted._id));
